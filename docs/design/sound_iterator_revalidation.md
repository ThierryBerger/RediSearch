...
The C code holds a single `*mut IndexReader` across lock boundaries. At the FFI level, the
reader is always stored as a `SuspendedReader` (the default state when no lock is held).

```rust
// FFI reader enum (existing pattern, unchanged structure)
pub enum IndexReader {
    Full(SuspendedReader<Full>),
    // ... other variants
}
```

FFI functions that need to read (called while the lock is held) transmute to `ActiveReader`
for the duration of the call. `IndexReader_Revalidate` performs `resume`. The lock
management remains in C, and the Rust side correctly models the aliasing.

The `unsafe` is confined to:
1. The `transmute` calls in `suspend`/`resume` (justified by `#[repr(C)]` + layout
   compatibility).
2. The FFI boundary functions (already `unsafe extern "C"`).
...
Resume **already exists** — the `Revalidate` mechanism. The Rust-side `resume` replaces
the current `needs_revalidation` + `refresh_buffer_pointers` with a proper state
transition from `SuspendedReader` to `ActiveReader`. No C-side suspend call is needed
(see next section).
